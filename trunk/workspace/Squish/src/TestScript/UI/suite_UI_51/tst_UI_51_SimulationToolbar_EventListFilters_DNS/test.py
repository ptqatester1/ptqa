...
def configPC0():
    ...
    pc0.desktop.ipConfiguration.setIPConfiguration("1.1.1.1", "255.0.0.0", "", "1.1.1.2")
...
def configServer0():
    ...
    server0.services.dns.on()
    server0.services.dns.addA("www.cisco.com", "1.1.1.2")

def checkPoint():
    util.clickOnSimulation()
    waitForObject(":CAppWindowBase")
    sendEvent("QMoveEvent", ":CAppWindowBase", 22, -13, 444, 0)
    eventListFilters.checkFilters('DNS', clearFilters=True)

    pc0.select()
    pc0.desktop.applications.webBrowser()
    pc0.desktop.webBrowser.browse("www.cisco.com")

    snooze(3)
    util.clickOnWorkspace(pc0.x + 10, pc0.y + 10)
    util.textCheckPoint(PDUConst.OSI_EXPLANATION, "1. The DNS client sends a DNS query to the DNS server.")